---
id: IND-002
name: "FEC - Frequência Equivalente de Interrupção por Unidade Consumidora"
...
# FEC - Frequência Equivalente de Interrupção por Unidade Consumidora (ID: IND-002)

Este Fact Sheet descreve o indicador de desempenho **FEC** (Frequência Equivalente de Interrupção por Unidade Consumidora) integrado ao catálogo mestre da **PowerUp OKC (Open Knowledge Catalog)**. Ele atua como uma métrica de controle de governança em total conformidade com as diretrizes do metamodelo v4 da **SAP LeanIX**.

## 1. Descrição Geral e Metodologia de Medição
Mede a frequência média de interrupções de longa duração (maior ou igual a 3 minutos) sofridas pelas UCs faturadas no conjunto no período mensal civil, servindo como métrica primária de confiabilidade de fornecimento de serviço da distribuidora.
