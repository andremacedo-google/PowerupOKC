---
id: IAA-020
name: "Otimizador de Descrições de Cargos"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Otimizador de Descrições de Cargos (IAA-020)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Otimizador de Descrições de Cargos**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Otimizador de Descrições de Cargos** é classificado arquiteturalmente como um **No-code Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Cognitivo (RAG)**. 

Sua missão de negócio é:
> Auxilia na redação e padronização inclusiva de descrições de cargo livres de vieses de linguagem, conectando o perfil da vaga à taxonomia corporativa.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Business Partner (BP) de RH / Recrutador Técnico**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Requisição de Vaga, Diretrizes de DEI`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **SAP SuccessFactors Recruiting**.
...
* **Business Capability (Nível 3):** `[/business-capabilities/cap-recrutamento.md]`
...
* **Business Application (System of Truth - SOT):** `[/business-applications/app-sap-successfactors-recruiting.md]`

---

## 5. Personalização (Sugestões de Conversa)

1. *"Como o Otimizador de Descrições de Cargos realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Otimizador de Descrições de Cargos?"*
3. *"Gerar um relatorio de analise do Otimizador de Descrições de Cargos."*
