---
id: IAA-051
name: "Lançador de Provisões de Fechamento"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Lançador de Provisões de Fechamento (IAA-051)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Lançador de Provisões de Fechamento**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Lançador de Provisões de Fechamento** é classificado arquiteturalmente como um **Data Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Create (C)**. 

Sua missão de negócio é:
> Executa lançamentos automáticos de provisões e acréscimos recorrentes de fechamento no Livro Razão com base em contratos cadastrados e regras operacionais.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Analista Contábil / Coordenador de Contabilidade**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Lançamento Contábil, Provisão Financeira`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **SAP S/4HANA (FI-GL)**.
...
1. *"Como o Lançador de Provisões de Fechamento realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Lançador de Provisões de Fechamento?"*
3. *"Gerar um relatorio de analise do Lançador de Provisões de Fechamento."*
