---
id: IAA-062
name: "Sincronizador de Campanhas e Interações"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Sincronizador de Campanhas e Interações (IAA-062)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Sincronizador de Campanhas e Interações**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Sincronizador de Campanhas e Interações** é classificado arquiteturalmente como um **Data Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Create / Update (C/U)**. 

Sua missão de negócio é:
> Atualiza e cria automaticamente novos registros de campanhas, interações de e-mail e status de oportunidades comerciais no CRM após ações e gatilhos de engajamento do usuário final.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Analista de CRM / Planejador de Campanhas / Gestor de Contas**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Campanha de Vendas (DO-129), Lead, Oportunidade, Histórico de Contato`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **Salesforce CRM**.
...
* **Business Capability (Nível 3):** `[/business-capabilities/cap-gestao-de-campanhas-de-marketing.md]`
* **Organization User Group (Team):** `[/organizations/teams/org-analista-de-crm.md]`
* **Data Objects Relacionados:** `[/data-objects/do-129.md]`
* **Business Application (System of Truth - SOT):** `[/business-applications/app-salesforce-crm.md]`

---

## 5. Personalização (Sugestões de Conversa)

1. *"Como o Sincronizador de Campanhas e Interações realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Sincronizador de Campanhas e Interações?"*
3. *"Gerar um relatorio de analise do Sincronizador de Campanhas e Interações."*
