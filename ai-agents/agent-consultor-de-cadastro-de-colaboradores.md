---
id: IAA-015
name: "Consultor de Cadastro de Colaboradores"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Consultor de Cadastro de Colaboradores (IAA-015)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Consultor de Cadastro de Colaboradores**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Consultor de Cadastro de Colaboradores** é classificado arquiteturalmente como um **No-code Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Read (R)**. 

Sua missão de negócio é:
> Realiza busca semântica em linguagem natural e consulta instantânea de informações cadastrais, contatos, cargo e estrutura de reporte do colaborador direto do ERP.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Analista de Administração de Pessoal / Business Partner (BP) de RH**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Colaborador (DO-183)`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **SAP SuccessFactors Employee Central**.
...
* **Business Capability (Nível 3):** `[/business-capabilities/cap-administracao-de-funcionarios.md]`
* **Organization User Group (Team):** `[/organizations/teams/org-analista-de-administracao-.md]`
* **Data Objects Relacionados:** `[/data-objects/do-183.md]`
* **Business Application (System of Truth - SOT):** `[/business-applications/app-sap-successfactors-employee-central.md]`

---

## 5. Personalização (Sugestões de Conversa)

1. *"Como o Consultor de Cadastro de Colaboradores realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Consultor de Cadastro de Colaboradores?"*
3. *"Gerar um relatorio de analise do Consultor de Cadastro de Colaboradores."*
