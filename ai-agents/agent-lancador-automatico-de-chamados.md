---
id: IAA-002
name: "Lançador Automático de Chamados"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Lançador Automático de Chamados (IAA-002)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Lançador Automático de Chamados**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Lançador Automático de Chamados** é classificado arquiteturalmente como um **No-code Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Create (C)**. 

Sua missão de negócio é:
> Cria e classifica automaticamente chamados de suporte de primeiro nível (N1) diretamente no sistema ITSM com base em formulários estruturados preenchidos pelos usuários.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Analista de Service Desk (N1) / Operador de Service Desk**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Ticket de Incidente, Requisição de Serviço`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **ServiceNow ITSM**.
...
* **Organization User Group (Team):** `[/organizations/teams/org-analista-de-service-desk-n.md]`
* **Data Objects Relacionados:** `[/data-objects/do-generic.md]`
* **Business Application (System of Truth - SOT):** `[/business-applications/app-servicenow-itsm.md]`

---

## 5. Personalização (Sugestões de Conversa)

1. *"Como o Lançador Automático de Chamados realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Lançador Automático de Chamados?"*
3. *"Gerar um relatorio de analise do Lançador Automático de Chamados."*
