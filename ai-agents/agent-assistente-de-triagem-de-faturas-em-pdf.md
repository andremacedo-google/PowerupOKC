---
id: IAA-055
name: "Assistente de Triagem de Faturas em PDF"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Assistente de Triagem de Faturas em PDF (IAA-055)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Assistente de Triagem de Faturas em PDF**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Assistente de Triagem de Faturas em PDF** é classificado arquiteturalmente como um **No-code Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Cognitivo (OCR/NLP)**. 

Sua missão de negócio é:
> Processa faturas recebidas por e-mail, extraindo metadados críticos via OCR/NLP (CNPJ, valor, vencimento) e realizando cruzamento semântico com fornecedores homologados para validação prévia de contas a pagar.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Analista de Contas a Pagar / Assistente Financeiro**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Fatura de Fornecedor (PDF), E-mail de Entrada, Registro de Fornecedor`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **SAP S/4HANA (FI-AP) / Mail Server**.
...
1. *"Como o Assistente de Triagem de Faturas em PDF realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Assistente de Triagem de Faturas em PDF?"*
3. *"Gerar um relatorio de analise do Assistente de Triagem de Faturas em PDF."*
