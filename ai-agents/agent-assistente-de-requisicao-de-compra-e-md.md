---
id: IAA-027
name: "Assistente de Requisição de Compra e MD"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Assistente de Requisição de Compra e MD (IAA-027)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Assistente de Requisição de Compra e MD**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Assistente de Requisição de Compra e MD** é classificado arquiteturalmente como um **No-code Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Create (C)**. 

Sua missão de negócio é:
> Apoia o requisitante de forma transacional no preenchimento de campos de requisições de compras e na padronização dos metadados técnicos.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Requisitante de Compras / Gestor de Contratos de TI**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Requisição de Compra, Memorial Descritivo (MD)`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **Coupa / ServiceNow**.
...
* **Business Capability (Nível 3):** `[/business-capabilities/cap-processamento-de-requisicoes-de-compra.md]`
* **Organization User Group (Team):** `[/organizations/teams/org-requisitante-de-compras.md]`
* **Data Objects Relacionados:** `[/data-objects/do-generic.md]`
* **Business Application (System of Truth - SOT):** `[/business-applications/app-coupa-servicenow.md]`

---

## 5. Personalização (Sugestões de Conversa)

1. *"Como o Assistente de Requisição de Compra e MD realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Assistente de Requisição de Compra e MD?"*
3. *"Gerar um relatorio de analise do Assistente de Requisição de Compra e MD."*
