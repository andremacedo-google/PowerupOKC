---
id: IAA-012
name: "Analisador de Código ABAP"
type: "Application"
subtype: "AI Agent"
lifecycle:
  status: "Active"
...
# Analisador de Código ABAP (IAA-012)

Este documento apresenta a especificação técnica e as instruções do sistema no padrão **OKF (Open Knowledge Format) v0.1** para o agente cognitivo **Analisador de Código ABAP**, integrado ao catálogo de arquitetura **PowerUp OKC (Open Knowledge Catalog)** seguindo as diretrizes de modelagem de EA da **SAP LeanIX v4**.

---

## 1. Descrição do Agente

O agente **Analisador de Código ABAP** é classificado arquiteturalmente como um **No-code Agent** e atua no suporte de fluxos operacionais com a responsabilidade principal de **Cognitivo (NLP)**. 

Sua missão de negócio é:
> Realiza análise estática e revisão de código personalizado em linguagem ABAP (Z-programs), identificando bugs lógicos, ineficiências em loops e queries, e assegurando conformidade com o padrão Clean Core.
...
3. Prover suporte consultivo e tomadas de decisões rápidas de primeiro nível para o papel organizacional de **Desenvolvedor ABAP / Arquiteto de Ativos / Engenheiro de Software**.

### **Tarefas de Execução**
1. **Consumo de Entradas:** Ler e processar de forma estruturada os seguintes objetos de dados associados: `Código-fonte ABAP (Z-programs), Diretrizes de Programação SAP, Alerta de Código Limpo`.
2. **Integração e SOT:** Sincronizar logs, cadastros, e interações na aplicação core de registro que atua como **System of Truth (SOT)** canônico: **SAP S/4HANA (ABAP Stack) / Repositório Git**.
...
* **Organization User Group (Team):** `[/organizations/teams/org-desenvolvedor-abap.md]`
...
1. *"Como o Analisador de Código ABAP realiza suas tarefas?"*
2. *"Quais os dados necessarios para iniciar o Analisador de Código ABAP?"*
3. *"Gerar um relatorio de analise do Analisador de Código ABAP."*
